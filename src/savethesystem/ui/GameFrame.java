    ...
    public GameFrame(User user) {
        ...
        if (sessionId == -1) {
            JOptionPane.showMessageDialog(this, 
                "Session gagal dibuat! Pastikan database nyala dan koneksi benar.");
            this.dispose();
            return;
        }
...
//        updateUI();
    ...
    if (stability <= 0 || performance <= 0 || security <= 0) {
        ...
        scoreDAO.saveScore(sessionId, score, round - 1);

        // ✅ FINISH session supaya tidak ada ACTIVE yang nyangkut
        SessionDAO sessionDAO = new SessionDAO();
        sessionDAO.finishSession(sessionId);
        
        GameOverFrame go = new GameOverFrame(currentUser, sessionId, score);
        go.setVisible(true);
        this.dispose(); // tutup GameFrame
}
}
